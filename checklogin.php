<?php
	...
	$query = mysql_query("SELECT * FROM uzytkowniki WHERE imie='$username'"); //Query the users table if there are matching rows equal to $username
	...
	if($exists > 0) //IF there are no returning rows or no existing username
	{
		while($row = mysql_fetch_assoc($query)) //display all rows from query
		{
			...
			$table_password = $row['haslo']; // the first password row is passed on to $table_users, and so on until the query is finished
		}
		if(($username == $table_users) && ($password == $table_password)) // checks if there are any matching fields
		{
				if($password == $table_password)
				{
					$_SESSION['imie'] = $username; //set the username in a session. This serves as a global variable
